...
- Crypto Offset :: 1 bit: Only used for transport mode. If set, the first
  4 bytes of the following Layer 4 Header are authenticated, but not encrypted.
  This bit MUST be set to 0 on tunnel mode. The receiver MUST drop Tunnel mode
  packets with this bit set. (NOTE: Or, the complete first L4 header is not encrypted.
  What's better???)
...
UDP Encapsulation packet format is identical to that of ESP. It is
negotiated using EESP IKEv2, as specified in
[I-D.klassert-ipsecme-eesp-ikev2]. The EESP Base Header also begins
with a 32-bit SPI. The SPI namespace is shared between IPsec SAs for
EESP and ESP. The SPI and destination address combination MUST be
unique across both ESP and EESP.

UDP Encapsulation of EESP allows the use of the source port for entropy
in ECMP.

* IKEv2 Negotiation

EESP IKEv2 negotiation is specified in
I-D.klassert-ipsecme-eesp-ikev2.
