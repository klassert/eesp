...
** AAD Construction
Authentication of data integrity and origin covers the Base
Header, any Optional Headers (if present), the SPI, Sequence Number,
and Initialization Vector (if present), the Payload Info Header
(if present), Payload Data, and any Padding (if present).
...
by the Crypt Offset is transmitted in the clear when encryption
is used, but is still included in the AAD.
...
#+begin_src
  ...
  |                              SPI                              | ege
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ rity
  |  Crypt Offset(2)  |Opt Len (4)|POffset(20)|CryptOff(7)| F | R | Pro
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ tec
