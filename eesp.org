...
The Crypt Offset and the Session ID can be used to cover the PSP use case.
...
The ~Payload Info Header~ is encrypted by default, and therefore
...
This document defines 3 initial standard option types, ~Pad1 Option~,
~PadN Option~, and ~Crypt Offset Option~.
...
[[Full and Optimized Packet Formats]]). The packet format depends on
...
The two possible packet formats are described in this section.
...
used, ommiting the ~Payload Info Header~.
...
~Payload Info Header~ MUST be used.
...
can be inferred from the decrypted payload. Table [[eesp-format-by-mode]]
summarizes which modes use which format. In this table, "Full" means
that the ~Payload Info Header~ is present, and "Optimized" means that
the ~Payload Info Header~ is absent (elided).
...
- Name :: [[https://git.kernel.org/pub/scm/linux/kernel/git/klassert/linux-stk.git/log/?h=ipsec-next-eesp-v3][XFRM-EESP-v3]]

- Description :: An initial Kernel IPsec implementation of the EESP.
