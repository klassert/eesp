...
additional Replay Subspace ID (see [[Session ID as Replay Subspace ID]]).
...
** AAD Construction
The authentication of data integrity and data origin covers
the Base Header, any Optional Headers present, SPI,
Sequence Number when present and Initialization Vector when present.
...
Payload Info Header when present and Payload Data transmitted
in plain aswell.

#+caption: EESP AAD with Crypt Offset
...
EESP. This document updates [[RFC4106]] and [[RFC4543]] for
EESP-specific Additional Authenticated Data (AAD) handling when
using AES-GCM. It also updates AAD processing for
ENCR_CHACHA20_POLY1305, as specified in [[RFC7634]].
...
| <l>                 |    <c>     |    <c>    |      <c>       |     <c>      |    <c>     |
|---------------------+------------+-----------+----------------+--------------+------------|
| Base Header         |     8      |     M     |                |      Y       |  plain AAD |
| Options             |  variable  |     O     |                |      Y       |  plain AAD |
| Sequence Number     |     8      |     O     |                |      Y       |  plain ADD |
| IV                  |  variable  |     O     |                |      Y       |  plain ADD |
