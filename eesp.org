...
addition to the IPsec end-point host system that use it to
...
for the IPsec end-point host system.
...
The ~Host Header~ follows the ~Base Header~ and ~Options~ field.
...
basis.
# Note from Tobias: So if you use a flow identifier, an SA can't transport multiple flows
# anymore (e.g. tunnel mode to different remote hosts, or even different
# ports/protocols to the same host)?  If so, then I don't get how this
# could be used to implement the "Sub-Child SA" feature as there every
# packet will potentially be for a different sub-child and have a
# different flow identifier value.
