...
parsing engines of middle boxes such as routers or firewalls and the
as well as the IPsec end-point host.
...
The ~Payload Data~ follows these 3 header sections, and has
...
Finally, packet ends with an optional ~Integrity Check Value~ (ICV)
(see Section 3.3.2 of [[RFC4303]]). The length of this ICV depends on the
Crypto suite.


*** Full and Optimized Packet Formats

# :NOTE: maybe move this below out of this generic description section
...
The ~Host Header~ follows the ~Base Header~ and ~Options~ field is the ~Host Header~
containing an optional ~Sequence Number~ and an optional
~Initialization Vector~, and the format is shown below.
