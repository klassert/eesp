...
number along with a 6-byte field consisting of two bytes Session ID
and 4 bytes Security Parameters Index (SPI).
The optional part of the Base Header defines Options to store
additional information, like integrity protected flow identifiers
that can be used for flow classification.

The ~Host Header~ and optional ~Options~ field follow the ~Base
Header~. The ~Host Header~ is used to route the packet to the correct
Crypto context and support replay protection.
...
Additionally, when IPv4 or IPv6 tunnel mode is used the ~Payload Info
Header~ MAY be elided. In this optimized mode the payload will
...
inner packet, so a pad length field is also not needed as it can be
derived.

The packet format without the ~Payload Info Header~ is the optimized
EESP packet fomat, while the packet format containing the ~Payload
Info Header~ is the full EESP packet format. Which of these two
formats are chosen is encoded in the ~Packet Format~ bit in the
Base Header.
...
Following the ~Payload Data~ and optional ~Padding~ is the optional
~Integrity Check Value~ (ICV) field which completes the packet (see
Section 3.3.2 of [[RFC4303]]). The length of this ICV depends on the
Crypto suite.

[[eesp-optimized-packet-format]] illustrates the resulting packet
format for use with IPv4 or IPv6 Tunnel Mode, and [[eesp-full-packet-format]]
for use in all other modes of operation.
...
alignment is dictated by the packet format. For the non-optimized
...
Following the ~Base Header~ and ~Options~ field is the Host Header
for e.g. optional Sequence Number and Initialization Vector.
When present, the Sequence Number is a full 64bit
sequence number. EESP only support 64bit sequence numbers, a.k.a ESN
and transmits the entire sequence number on each packet.
The actual size of the Initialization Vector depends on the
choice of the cipher suite.

The host Header  is defined as in [[host-header]]. Detailed
specification of the Sequence Number and Initialization Vector
...
The Payload Info Header is present in the non-optimized packet
...
- F[lags] :: 2-bits: Flags used for inline crypto signaling such as the
  ...
  :NOTE: Is this used and thus still required by PSP, or can it be
  removed?
...
#+begin_src
    ...
    -------   ------------------------------    ---------------
          1   Pad1                              [this document]
          2   PadN                              [this document]
          3   Crypt Offset                      [this document]
          4   FID                               [this document]
      5-111   Unassigned                        [this document]
    112-127   Private                           [this document]
#+end_src
